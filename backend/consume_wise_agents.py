...
class Agents(): 
   ...
   def summarizer_agent(self):
       return Agent(
           role='Summarizer',
           goal='Summarize all the relevant information into one JSON format',
           backstory="""An expert in summarizing data and providing insightful suggestions based on historical data.""",
           tools=[
               DBTools.get_user_data,
               DBTools.get_user_activities,
               DBTools.get_user_suggestions
           ],
           verbose=True
       )
   ...
   def db_agent(self):
       return Agent(
           role='DBExpert',
           goal='Able to do all the CRUD operations to the DB',
           backstory="""An expert in doing all the CRUD operations to the DB""",
           tools=[
               DBTools.get_all_users,
           ],
           verbose=True
       )